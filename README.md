...
| Thumbnails (or other project data) still look wrong *after* fixing the sheet, even after clicking "Refresh Projects" | Stale/bad data got cached in browser localStorage before the fix went in, and the button is re-reading a corrupted cache instead of truly bypassing it | Open DevTools console on the site and run `localStorage.removeItem("marketplace_projects_cache"); localStorage.removeItem("marketplace_projects_last_refresh");` (or `localStorage.clear()`), then reload |
| Site looks broken **only** inside the Google Sites embed (text way too spread out, bold text isn't bold) — but looks normal on the live GitHub Pages URL or in local dev | Google Sites cached an old snapshot of the embedded iframe and hasn't refetched it, so it's showing an older build's styling | Just reload the Google Sites page. If it persists, delete and re-insert the embed (Insert → Embed → By URL) rather than editing the existing one |
